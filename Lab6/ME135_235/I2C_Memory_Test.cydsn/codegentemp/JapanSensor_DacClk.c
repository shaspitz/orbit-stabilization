/*******************************************************************************
* File Name: JapanSensor_DacClk.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: JapanSensor_DacClk_Start
********************************************************************************
*
* Summary:
...
void JapanSensor_DacClk_Start(void) 
{
    ...
    JapanSensor_DacClk_CLKEN |= JapanSensor_DacClk_CLKEN_MASK;
	JapanSensor_DacClk_CLKSTBY |= JapanSensor_DacClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: JapanSensor_DacClk_Stop
********************************************************************************
*
* Summary:
...
void JapanSensor_DacClk_Stop(void) 
{
    ...
    JapanSensor_DacClk_CLKEN &= (uint8)(~JapanSensor_DacClk_CLKEN_MASK);
	JapanSensor_DacClk_CLKSTBY &= (uint8)(~JapanSensor_DacClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: JapanSensor_DacClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((JapanSensor_DacClk_CLKEN & JapanSensor_DacClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = JapanSensor_DacClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = JapanSensor_DacClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* JapanSensor_DacClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(JapanSensor_DacClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(JapanSensor_DacClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: JapanSensor_DacClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: JapanSensor_DacClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = JapanSensor_DacClk_GetSourceRegister();
    uint16 oldDivider = JapanSensor_DacClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = JapanSensor_DacClk_CLKEN & JapanSensor_DacClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(JapanSensor_DacClk_DIV_PTR, clkDivider);
                JapanSensor_DacClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                JapanSensor_DacClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(JapanSensor_DacClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((JapanSensor_DacClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(JapanSensor_DacClk_DIV_PTR, clkDivider);
				JapanSensor_DacClk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: JapanSensor_DacClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(JapanSensor_DacClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: JapanSensor_DacClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    JapanSensor_DacClk_MOD_SRC |= modeBitMask & (uint8)JapanSensor_DacClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: JapanSensor_DacClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    JapanSensor_DacClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(JapanSensor_DacClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: JapanSensor_DacClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return JapanSensor_DacClk_MOD_SRC & (uint8)(JapanSensor_DacClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: JapanSensor_DacClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = JapanSensor_DacClk_GetDividerRegister();
    uint8 oldSrc = JapanSensor_DacClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        JapanSensor_DacClk_MOD_SRC |= CYCLK_SSS;
        JapanSensor_DacClk_MOD_SRC =
            (JapanSensor_DacClk_MOD_SRC & (uint8)(~JapanSensor_DacClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: JapanSensor_DacClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return JapanSensor_DacClk_MOD_SRC & JapanSensor_DacClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: JapanSensor_DacClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    JapanSensor_DacClk_PHASE = clkPhase & JapanSensor_DacClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: JapanSensor_DacClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return JapanSensor_DacClk_PHASE & JapanSensor_DacClk_PHASE_MASK;
}
