...
def dyn_discrete(r0,w0,Ts):
    ...
    B=np.array([[0,0],[1+Ts,0],[0,0],[0,(1+Ts)/r0]])
    return A,B
...
def main():
    ...
    if hardware_in_loop:
        ...
        t_sim = np.linspace(0, 100000, 100000)
...
#         
        ...
        for k in range(N-1):
            ...
            #this is the formula for xhat given in Chapter 15 of 231B notes
            #xhat[:,k+1]=(A - B@Finf - Kinf@H@A)@(xhat[:,k]) + Kinf@H@A@(x[:,k])
            #using sys_sol_lin as a "measurement"
            xhat[:,k+1]=sys_sol_lin.y[:,k]
        ...
        #Add equilibrium values for filtered and controlled system
        for k in range(N):
            x[:,k] = x[:,k] + equil(k*Ts)
...
#         # x_sat_nl = [sys_sol_nl.y[0][i]*np.cos(sys_sol_nl.y[2][i]) for i in range(len(t_sim))]
        ...
        circle1 = plt.Circle((0, 0), R, color='b')
        ax.add_artist(circle1)
        plt.legend(['KF Model', 'Discrete Linear Model'],
                   loc='lower right')
        plt.show()
        '''
